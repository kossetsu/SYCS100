...
##Eric Walker
def bsearch(listA, s): #defines the function
    extant = False #sets a parameter to initiate the while loop
    startingPara = 0 #starting parameter is set to 0
    endingPara = len(listA) - 1 #ending parameter is set to the length of the list minus 1
    search = s #sets search to be the variable s
    while ((endingPara >= startingPara) and extant == False): #runs the function until the ending parameter is < the starting parameter and extant is equal to true
        midpoint = ((startingPara + endingPara) / 2) #sets the midpoint to equal the average of the starting and ending parameters
        if search < listA[midpoint]: #sets condition for the search value being less than the index of midpoint in listA
            endingPara = midpoint - 1 #if so, ending parameter will equal the midpoint minus 1
        elif search > listA[midpoint]: #sets condition for the search being greater than index midpoint of listA
            startingPara = midpoint + 1 #starting parameter will equal midpoint + 1
        elif search == listA[midpoint]: #checks if search value is the index midpoint at listA
            extant = True #if so extant will now be true
            return midpoint #the midpoint will be returned
    else:
        return -1 #if none is true, -1 will be returned
